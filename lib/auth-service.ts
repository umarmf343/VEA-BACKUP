...
class AuthService {
  ...
  private async purgeStaleRefreshTokens() {
    await dbManager.purgeExpiredRefreshTokens(Date.now())
  }
  ...
  private async persistRefreshToken(
    ...
    await dbManager.saveRefreshToken({
      id: refreshJti,
      userId: user.id,
    ...
    if (rotateFrom) {
      await dbManager.updateRefreshToken(rotateFrom, {
        rotated: true,
  ...
  private async revokeUserRefreshTokens(userId: string) {
    await dbManager.deleteUserRefreshTokens(userId)
  }

  private async createTokenPair(user: AuthenticatedUser, rotateFrom?: string): Promise<TokenPair> {
    await this.purgeStaleRefreshTokens()
    ...
    const accessTokenExpiresAt = computeExpiry(ACCESS_TOKEN_TTL_MS)
    const refreshTokenExpiresAt = computeExpiry(REFRESH_TOKEN_TTL_MS)

    await this.persistRefreshToken(user, refreshTokenId, refreshTokenExpiresAt, rotateFrom)
  ...
  async login(email: string, password: string): Promise<LoginResult> {
    ...
    await this.revokeUserRefreshTokens(userRecord.id)
  ...
  async refreshSession(refreshToken: string): Promise<RefreshResult> {
    ...
    const stored = await dbManager.getRefreshToken(payload.jti)
    ...
    if (Number.isNaN(expiresAtMs) || expiresAtMs <= Date.now()) {
      await dbManager.updateRefreshToken(payload.jti, { rotated: true })
    ...
    const tokens = await this.createTokenPair(user, payload.jti)
