...
export default function SuperAdminDashboard() {
  ...
  const handleBrandingUpload = async () => {
    try {
      ...
      await DatabaseManager.saveBranding(brandingData)
      ...
      DatabaseManager.triggerEvent("brandingUpdated", brandingData)
  ...
  const handleSaveReportCard = async () => {
    try {
      ...
      await DatabaseManager.saveReportCard(reportData)

      // Trigger real-time updates for teacher and parent panels
      DatabaseManager.triggerEvent("reportCardUpdated", reportData)
  ...
  const handleSaveUser = async () => {
    try {
      ...
      // Save to database with real-time updates
      await DatabaseManager.saveUser(userData)
      ...
      DatabaseManager.triggerEvent("userUpdated", userData)
  ...
  const handleDeleteUser = async (userId: number) => {
    if (confirm("Are you sure you want to delete this user?")) {
      try {
        ...
        DatabaseManager.triggerEvent("userDeleted", { userId })
  ...
  useEffect(() => {
    const loadAdminData = async () => {
      try {
        ...
        const [users, branding, systemHealth] = await Promise.all([
          ...
          DatabaseManager.getBranding(),
          DatabaseManager.getSystemHealth(),
        ])
        ...
        DatabaseManager.addEventListener("userUpdated", (userData) => {
          setUsers((prev) => {
        ...
        DatabaseManager.addEventListener("userDeleted", ({ userId }) => {
    ...
    return () => {
      DatabaseManager.removeAllEventListeners()
    }
  }, [])
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        const classesData = await DatabaseManager.getAllClasses()
        setClasses(classesData)

        // Load system settings
        const settings = await DatabaseManager.getSystemSettings()
        if (settings) {
          setSystemSettings(settings)
        }

        // Load users
        const usersData = await DatabaseManager.getAllUsers()
        setUsers(usersData)

        // Set up real-time listeners
        DatabaseManager.addEventListener("userUpdated", (data) => {
          setUsers((prev) => prev.map((user) => (user.id === data.id ? data : user)))
        })

        DatabaseManager.addEventListener("classUpdated", (data) => {
          setClasses((prev) => [...prev.filter((c) => c !== data.oldName), data.newName])
        })

        DatabaseManager.addEventListener("settingsUpdated", (data) => {
    ...
    // Cleanup listeners on unmount
    return () => {
      DatabaseManager.removeEventListener("userUpdated")
      DatabaseManager.removeEventListener("classUpdated")
      DatabaseManager.removeEventListener("settingsUpdated")
    }
  }, [])

  const handleSaveSystemSettings = async () => {
    try {
      await DatabaseManager.saveSystemSettings(systemSettings)
      DatabaseManager.triggerEvent("settingsUpdated", systemSettings)
  ...
  const handleAddClass = async (className: string) => {
    try {
      await DatabaseManager.addClass(className)
      const updatedClasses = await DatabaseManager.getAllClasses()
      setClasses(updatedClasses)
      DatabaseManager.triggerEvent("classUpdated", { newName: className, action: "added" })
    } catch (error) {
  ...
  const handleDeleteClass = async (className: string) => {
    try {
      await DatabaseManager.deleteClass(className)
      const updatedClasses = await DatabaseManager.getAllClasses()
      setClasses(updatedClasses)
      DatabaseManager.triggerEvent("classUpdated", { oldName: className, action: "deleted" })
    } catch (error) {
