...
// Purpose: Stable student overview with reliable polling + accessible UI.
// Fixes addressed:
// - Flickery / mismatched rendering (explicit client component, guarded states)
// - No refresh / retry path (manual refresh + 10s polling)
// - Missing error surfacing and focus management for screen readers
...
export default function StudentDashboard() {
  ...
  async function load() {
    ...
    try {
      ...
      const json = (await res.json()) as Partial<StudentData>;
    ...
    <section className="space-y-4">
      <header className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Your current tasks and notices at a glance.</p>
        </div>
